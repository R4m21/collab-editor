...
export async function flushSyncQueue(documentId: string): Promise<void> {
  ...
  console.log(`[SyncEngine] Flushing ${pending.length} queued updates for doc ${documentId}`)

  for (const item of pending) {
    if (item.retries >= MAX_RETRIES) {
      console.error(`[SyncEngine] Dropping update after ${MAX_RETRIES} retries`, item)
...
export async function queueUpdate(documentId: string, update: Uint8Array): Promise<void> {
  const db = getDB()
